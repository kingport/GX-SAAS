...
import { LoadingOutlined, InboxOutlined, DeleteOutlined } from '@ant-design/icons';
import './index.less';
...
class UploadElement extends React.PureComponent {
  ...
  updateImgurl = (initialImageUrl) => {
    if (initialImageUrl) {
      this.setState({
        imageUrl: `${window.origin}/saas/api/v1/common/showFile?key=${initialImageUrl}`,
      });
    }
  };
  ...
  render() {
    const { uploadTxt = '', disabled, canRemove = false } = this.props;
    ...
    const uploadButton = (
      <div>
        {loading ? (
          ...
          <InboxOutlined style={{ fontSize: 48, color: '#40a9ff', padding: '15px 0' }} />
        )}
    ...
    return (
      <div className="clearfix">
        <Dragger
          ...
          action={`${window.origin}/saas/api/v1/common/uploadFile`} // 上传的地址
          ...
          >
            {imageUrl ? (
              <div
                ...
                style={{ display: 'inline-block', width: 288, height: 148 }}
              >
                <img
                  src={imageUrl}
                  alt="avatar"
